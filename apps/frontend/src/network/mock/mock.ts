...
const filterChampionsListByRoles = (
  query: GetRandomChampionQuery | GetChampionsQuery,
): ChampionsList => {
  ...
  const championsListFiltered = roles.flatMap(
    (role) => championsByRole[role] || [],
  );
  return championsListFiltered;
};

const getChampions: GetChampionsResponse = ({
  query,
}: { query: GetChampionsQuery }): ChampionsList => {
...
const getRandomChampion: GetRandomChampion = ({
  query,
}: { query: GetRandomChampionQuery }): Champion => {
