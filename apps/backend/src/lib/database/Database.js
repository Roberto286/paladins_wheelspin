...
export class Database {
  ...
  executeScript(sqlScriptPath) {
    try {
      const script = readFileSync(sqlScriptPath, 'utf-8');
  ...
  getChampionsBy(fields, values) {
    return new Promise((resolve, reject) => {
      if (
        ...
        reject(
          new Error('Fields and values must be arrays of the same length.'),
        );
        return;
      }
  ...
  getAllChampions(reversed) {
    return new Promise((resolve, reject) => {
      this.db.all(
        `SELECT * FROM champions_view ${reversed ? 'ORDER BY id DESC' : ''}`,
        (err, rows) => {
          if (err) {
            reject(err);
          } else {
